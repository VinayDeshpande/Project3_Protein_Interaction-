					 	...
					 	if (d.length>0){
					 		...
					 		if (propertyName.indexOf("Unknown gene")>-1)
					 		{
								//console.log(propertyName); 	
							 	...
							 	//var arr = d3.tsv.parseRows(d[2][n])[0];
					...
					.on('load', function(d) { 
						...
						for(var i=0;i<d.length;i++)
						{
							for( key in d[i])
							{
								...
								//debugger;
								//console.log(geneName+" "+d[i][key.split("	").indexOf("CANCER_TYPE")];
								if(sex>-1)
								{
						...
						//console.log("Male Count "+male_count+" "+"Female Count "+female_count);
						//debugger;
					/*
					 	var count=0;
					 	if (d.length>0){
					    ...
					    //updateNodeColors();	
					*/	
					//console.log("Gene "+geneName+"Male Count "+(male_count)/(male_count+female_count)+" "+"Female Count "+(female_count)/(male_count+female_count));					
					});  
					})(request,i,p);
			} 
		}
